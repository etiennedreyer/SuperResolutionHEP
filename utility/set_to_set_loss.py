...
class SetToSetLoss:
    ...
    def get_loss_mask_part(self, part_mask_float):
        '''
        ...
        return not_q4, q2_q3_inf # shape (bs, max_part, max_part)



class SetToSetLossKinematics(SetToSetLoss):

    def __init__(self, config, max_part):
        super().__init__(config, max_part)
    ...
    def compute(self, input, batch, n_ray=0):
        ...
        not_q4, q2_q3_inf = self.get_loss_mask_part(batch['part_mask'].float())
...
class SetToSetLossIncidence(SetToSetLoss):

    def __init__(self, config, max_part):
        super().__init__(config, max_part)

    def compute(self, input, batch, kin_pred, n_ray=0):

        bs = input.size(0)
        target = batch['incidence_matrix'].transpose(1, 2)

        # apply cell mask, just to be sure
        input = input * batch['cell_mask'].unsqueeze(1)
        target = target * batch['cell_mask'].unsqueeze(1)

        input_reshaped = input.unsqueeze(1).expand(-1, self.max_part, -1, -1)
        target_reshaped = target.unsqueeze(2).expand(-1, -1, self.max_part, -1)
        cell_mask_reshaped = batch['cell_mask'].unsqueeze(1).unsqueeze(2).expand(-1, self.max_part, self.max_part, -1)

        kld = - target_reshaped * torch.log(input_reshaped + self.EPS)
        kld = kld.sum(-1) / cell_mask_reshaped.sum(-1)
        
        not_q4, q2_q3_inf = self.get_loss_mask_part(batch['part_mask'].float())
        pdist = kld * not_q4 + q2_q3_inf
        ...
        # kin loss
        kin_pred_reordered = kin_pred[torch.arange(bs)[:, None], assignment_indices[:, 1], :]

        pt_loss = F.mse_loss(kin_pred_reordered[:, :, 0], batch['part_pt'])
        eta_loss = F.mse_loss(kin_pred_reordered[:, :, 1], batch['part_eta'])
        phi_loss = (1 - torch.cos(kin_pred_reordered[:, :, 2] - batch['part_phi'])).mean()
        e_loss = F.mse_loss(kin_pred_reordered[:, :, 3], batch['part_dep_e']) # dep_e not e
        ...
        loss_componenets = {
            ...
            'eta_loss': eta_loss.item(),
            'phi_loss': phi_loss.item(),
            'kin_loss': e_loss.item() + pt_loss.item() + eta_loss.item() + phi_loss.item()
        }
